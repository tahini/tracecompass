 ...
 */
public class TmfActiveFilterHeader extends Composite {
     ...
     */
    public interface IActiveFilterHeaderListener {
        /**
         ...
         *            the selected filter
         */
        void filterSelected(TraceCompassFilter filter);
         ...
         *            the removed filter
         */
        void filterRemoved(TraceCompassFilter filter);
    }

    private final IActiveFilterHeaderListener fListener;
     ...
     */
    public TmfActiveFilterHeader(Composite parent, int style, IActiveFilterHeaderListener listener) {
        super(parent, style);
        fListener = listener;
     ...
     *            the filter to add
     */
    public void addFilter(TraceCompassFilter filter) {
        if (filter.getRegex().isEmpty()) {
            // Do not add empty regexes
            return;
        }
        addNewFilter(filter);
        fLayout.marginTop = 1;
     ...
     */
    public void removeFilter(TraceCompassFilter filter) {
        ...
        label.addMouseListener(new MouseAdapter() {
            ...
            public void mouseDown(MouseEvent e) {
                ...
                if (bounds.contains(e.x, e.y)) {
                    fListener.filterRemoved((TraceCompassFilter) label.getData());
                } else {
                    fListener.filterSelected((TraceCompassFilter) label.getData());
                    getParent().layout(true, true);
                }
            }
        });
